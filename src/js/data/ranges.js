 ...
 */
/**
 * How far down a row of the grid a position still opens.
 *
 * A chart is 169 squares, which nobody memorises. What players actually
 * carry is one boundary per row — "suited kings down to K9s under the gun,
 * K5s in the cutoff" — and the rule that the boundary slides further down
 * the later you sit. Returned as the weakest hand still opened, or null when
 * the row is empty or has a gap in it and so has no single boundary to name.
 */
export const BOUNDARY_ROWS = Object.freeze([
  { id: 'Ks', high: 'K', suited: true, label: 'suited king' },
  { id: 'Qs', high: 'Q', suited: true, label: 'suited queen' },
  { id: 'Js', high: 'J', suited: true, label: 'suited jack' },
  { id: 'Ts', high: 'T', suited: true, label: 'suited ten' },
  { id: 'Ao', high: 'A', suited: false, label: 'offsuit ace' },
  { id: 'Ko', high: 'K', suited: false, label: 'offsuit king' },
  { id: 'Qo', high: 'Q', suited: false, label: 'offsuit queen' },
]);

const RANK_CHARS_ASC = '23456789TJQKA';

export function rowBoundary(range, high, suited) {
  const highIndex = RANK_CHARS_ASC.indexOf(high);
  if (highIndex < 1) return null;
  const key = (loIndex) => high + RANK_CHARS_ASC[loIndex] + (suited ? 's' : 'o');

  let lowest = null;
  for (let lo = 0; lo < highIndex; lo++) {
    if (range.has(key(lo))) { lowest = lo; break; }
  }
  if (lowest === null) return null;
  // A boundary only means something if the row runs unbroken from there up.
  for (let lo = lowest; lo < highIndex; lo++) if (!range.has(key(lo))) return null;
  return key(lowest);
}
